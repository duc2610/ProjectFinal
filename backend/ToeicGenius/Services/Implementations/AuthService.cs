...
namespace ToeicGenius.Services.Implementations
{
	public class AuthService : IAuthService
	{
		...
		public async Task<string> ChangePasswordAsync(ChangePasswordDto changePasswordRequest, string userId)
		{
			...
			if (SecurityHelper.VerifyPassword(changePasswordRequest.NewPassword, user.PasswordHash))
			{
		...
		public async Task<string> ConfirmResetPasswordAsync(ResetPasswordConfirmDto resetPasswordConfirmDto)
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(user.PasswordHash) && SecurityHelper.VerifyPassword(resetPasswordConfirmDto.NewPassword, user.PasswordHash))
				{
					return "Mật khẩu mới không được trùng với mật khẩu cũ!";
				}
