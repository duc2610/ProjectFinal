...
import { Card, Button, Input, Table, Space, Popconfirm, Modal, Tag, Tabs, Switch, Select, message, Tooltip } from "antd";
...
export default function ExamManager() {
    ...
    const questionColumns = [
        ...
        { title: "Question", dataIndex: "question", key: "question" },
        {
            ...
            width: 100,
            align: "center",
            render: v => <div className={styles.answerBadge}>{v}</div>,
        },
        {
            title: "Status",
            dataIndex: "isActive",
            ...
            width: 120,
            render: isActive =>
                isActive ? <Tag color="green">Active</Tag> : <Tag>Disabled</Tag>,
        },
        {
            title: "Actions",
            key: "actions",
                        ...
                        onConfirm={() => {
                            setQuestionsByExam(prev => {
                                ...
                                Object.keys(copy).forEach(
                                    k => (copy[k] = copy[k].filter(q => q.id !== row.id))
                                );
                                return copy;
                            });
                        ...
                        <Button
                            ...
                            onClick={() => {
                                ...
                                if (isBank) {
                                    ...
                                    setQuestionsByExam(prev => {
                                        const copy = { ...prev };
                                        copy[row.examId] = (copy[row.examId] || []).map(q =>
                                            q.id === row.id
                                                ? { ...q, isActive: !(q.isActive !== false) }
                                                : q
                                        );
