...
// buildQuestions dùng chung với ExamSelection / PracticeSW / TestHistoryTab
// Quan trọng: xử lý đúng speaking_group để màn làm bài Speaking hiển thị giống lúc bắt đầu / tiếp tục
      ...
      if (isSpeakingPart && tq.isGroup && tq.questionGroupSnapshotDto) {
        ...
        group.questionSnapshots?.forEach((qs, idx) => {
          subQuestions.push({
            ...
            globalIndex: globalIndex++, // Mỗi sub-question có globalIndex riêng
            type: "group",
            question: qs.content,
...
export default function ResultScreen() {
  ...
  // Hàm xử lý quay lại - quay về trang list test tương ứng
  const handleGoBack = () => {
  ...
  const columns = [
    ...
    {
      title: "Đáp án & Kết quả",
      key: "answerSummary",
      width: 220,
      render: (_, row) => {
        const val = row.isCorrect;
        const userLabel = row.userAnswer || "—";
        const userText = row.userAnswerText || "";
        const correctLabel = row.correctAnswer || "—";
        const correctText = row.correctAnswerText || "";
        return (
        <div>
    ...
    },
    {
      ...
      render: (_, row) => (
        <div style={{ display: "flex", gap: 8 }}>
          ...
          {/* Xác định nhóm câu: dùng flag isGroup trong row */}
          {isQuestionReported(row.testQuestionId, row.questionId, row.isGroup === true) ? (
            ...
            <Button
              ...
              onClick={() => {
                ...
                // Kiểm tra xem câu hỏi đã được báo cáo chưa (dùng flag isGroup trong row)
                if (isQuestionReported(row.testQuestionId, row.questionId, row.isGroup === true)) {
                  message.info("Câu hỏi này đã được báo cáo rồi");
                  return;
                }
                const rawQuestion = row.question || row.content || "";
  ...
  // Khối thông tin chi tiết theo từng tab
  const renderGlobalDetailTiles = () => {
    ...
    // Thông tin chi tiết theo từng tab
    if (selectedSection === "listening") {
      // Thông tin chi tiết phần Nghe
      const listeningStats = practiceLrStats.listening;
      const listeningAccuracy = listeningStats.total > 0 
        ? Math.round((listeningStats.correct / listeningStats.total) * 100) 
        : 0;
      
      tiles.push({
        label: "Tổng số câu phần Nghe",
        value: listeningStats.total,
        color: "#0958d9",
      });
      tiles.push({
        label: "Câu đã làm",
        value: listeningStats.answered,
        color: "#1d39c4",
      });
      tiles.push({
        label: "Câu chưa làm",
        value: listeningStats.unanswered,
        color: "#fa8c16",
      });
      tiles.push({
        label: "Đúng",
        value: listeningStats.correct,
        color: "#389e0d",
      });
      tiles.push({
        label: "Sai",
        value: listeningStats.wrong,
        color: "#cf1322",
      });
      tiles.push({
        label: "Độ chính xác phần Nghe",
      ...
      const readingAccuracy = readingStats.total > 0 
        ? Math.round((readingStats.correct / readingStats.total) * 100) 
        : 0;
      
      tiles.push({
        label: "Tổng số câu phần Đọc",
        value: readingStats.total,
        color: "#0958d9",
      });
      tiles.push({
        label: "Câu đã làm",
        value: readingStats.answered,
        color: "#1d39c4",
      });
      tiles.push({
        label: "Câu chưa làm",
        value: readingStats.unanswered,
        color: "#fa8c16",
      });
      tiles.push({
        label: "Đúng",
        value: readingStats.correct,
        color: "#389e0d",
      });
      tiles.push({
        label: "Sai",
        value: readingStats.wrong,
        color: "#cf1322",
      });
      tiles.push({
        label: "Độ chính xác phần Đọc",
      ...
      >
        ...
        >
          <Title level={5} style={{ marginBottom: 12, textAlign: "center" }}>
            Thông tin chi tiết
            {selectedSection === "listening" && " - Phần Nghe"}
            {selectedSection === "reading" && " - Phần Đọc"}
            {selectedSection === "writing" && " - Phần Viết"}
            {selectedSection === "speaking" && " - Phần Nói"}
            {selectedSection === "overall" && " - Tổng quan"}
          </Title>
          <div
            style={{
              display: "flex",
        ...
        {reportQuestion && (
          <>
            <div style={{ marginBottom: 16 }}>
              ...
              >
                {(() => {
                  const raw = formatQuestionText(
                    reportQuestion.question || reportQuestion.content || ""
                  );
                  if (raw && raw.trim() && raw.trim() !== "—") {
                    return raw;
                  }
                  const parts = [];
                  if (reportQuestion.index) {
                    parts.push(`Câu ${reportQuestion.index}`);
                  }
                  if (reportQuestion.partTitle) {
                    parts.push(reportQuestion.partTitle);
                  }
                  const header = parts.length ? parts.join(" - ") : "Câu hỏi";
                  return `${header} (Câu hỏi không có nội dung văn bản, chỉ hình ảnh/âm thanh).`;
                })()}
              </div>
            </div>
            <div style={{ marginBottom: 16 }}>
          ...
          <div>
            ...
            {selectedQuestionDetail.passage && (
              ...
              >
                ...
                <div style={{ marginTop: 8, whiteSpace: "pre-wrap" }}>
                  {formatQuestionText(selectedQuestionDetail.passage)}
                </div>
              </div>
            )}
            ...
            <div style={{ 
              ...
              {isQuestionReported(
                ...
                selectedQuestionDetail.isGroup === true
              ) ? (
                ...
                <Button
                  icon={<FlagOutlined />}
                  onClick={() => {
                    // Kiểm tra xem câu hỏi đã được báo cáo chưa (dùng flag isGroup)
